...
public class advLeaderboard {
	...
	public advLeaderboard() throws FileNotFoundException{
		...
		Scanner input = new Scanner(new File("src/MarioGameTestingPlatForm/MarioLeaderBoard.txt"));
		int position = 0;
		while (input.hasNext()) {
			String l = input.next();
	...
	public void writeLeaderboard() throws FileNotFoundException{
		PrintStream output = new PrintStream(new File("src/MarioGameTestingPlatForm/MarioLeaderBoard.txt"));
